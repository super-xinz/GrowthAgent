...
func (s *SearchAction) Search(ctx context.Context, keyword string, filters ...FilterOption) ([]Feed, error) {
	// Context() replaces Rod's page context, so apply Timeout afterwards. The old
	// order silently removed the 60s timeout and left a stuck search hanging until
	// the HTTP server cancelled it five minutes later.
	page := s.page.Context(ctx).Timeout(55 * time.Second)
	var result string
	var validationErr error

	// Rod's Must* helpers panic on browser errors. Convert those panics into a
	// normal error so the API can recover without a five-minute Gin panic trace.
	panicErr := rod.Try(func() {
		...
		if len(filters) > 0 {
			...
			for _, filter := range filters {
				...
				if err != nil {
					validationErr = fmt.Errorf("筛选选项转换失败: %w", err)
					return
				}
			...
			for _, filter := range allInternalFilters {
				if err := validateInternalFilterOption(filter); err != nil {
					validationErr = fmt.Errorf("筛选选项验证失败: %w", err)
					return
				}
			}
	...
	if panicErr != nil {
		return nil, fmt.Errorf("小红书搜索页加载失败: %w", panicErr)
	}
