...
func makeFeedDetailURL(feedID, xsecToken string) string {
	// GrowthAgent obtains feed IDs and xsec tokens from the web search endpoint.
	// Xiaohongshu validates the token together with its source; presenting a
	// search token as ``pc_feed`` makes otherwise valid notes show the generic
	// "This Page Isn't Available Right Now" interstitial.
	return fmt.Sprintf(
		"https://www.xiaohongshu.com/explore/%s?xsec_token=%s&xsec_source=pc_search&source=web_search_result_notes",
		feedID,
		xsecToken,
	)
}
