    ...
    async with httpx.AsyncClient(headers=HEADERS, timeout=15, follow_redirects=True) as client:
        for item in dict.fromkeys(urls[:20]):
            try:
                ...
                if response.status_code >= 400 or len(response.text) < 40: continue
                title, content = (item.rsplit("/",1)[-1] or "README", response.text[:30000]) if github else clean_html(response.text)
                if content:
                    results.append({"url": str(response.url), "type": "github" if github else "website", "title": title,
