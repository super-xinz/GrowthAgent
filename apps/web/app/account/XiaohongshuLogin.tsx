...
export default function XiaohongshuLogin({initialStatus}:{initialStatus:Status|null}){
  ...
  return <section className="card account-card">
    <div className="account-status"><span className={`account-dot ${status?.is_logged_in?"online":""}`}/><div><strong>{status?.is_logged_in?"已连接小红书":"尚未登录"}</strong><p>{status?.is_logged_in?(status.username||"自动搜索与低频评论已可用"):"使用小红书 App 扫码登录，不需要 API Key。"}</p></div></div>
    ...
    <div className="actions">{status?.is_logged_in?<button className="button secondary" disabled={busy} onClick={logout}>退出当前账号</button>:<button className="button" disabled={busy} onClick={showQr}>{qr?"刷新二维码":"显示登录二维码"}</button>}</div>
  </section>;
}
