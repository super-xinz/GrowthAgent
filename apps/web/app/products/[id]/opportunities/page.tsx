...
export default async function Opportunities({params}:{params:Promise<{id:string}>}){
  ...
  return <>
    ...
    <div className="card"><table className="table"><thead><tr><th>对话内容</th><th>社区</th><th>用户意图</th><th>评分</th><th>策略与状态</th><th>回复草稿</th></tr></thead><tbody>
      ...
      {!rows.length&&<tr><td className="empty" colSpan={6}>暂无机会数据。构建 Product Brain 后可运行 <code>make seed</code> 导入本地测试数据。</td></tr>}
    </tbody></table></div>
  </>;
}
